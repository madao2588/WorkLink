...
def _ensure_department(
    ...
    *,
    ...
    if db.get(Department, department_id):
        return
    db.add(Department(id=department_id, name=name, parent_id=parent_id, sort_no=sort_no))
    db.commit()


def _ensure_madao_user(db: Session) -> None:
    if db.scalar(select(User).where(User.login_id == "madao")):
        return
    ...
    db.add_all(
        [
            Message(
                ...
                conversation_id="c_1",
                sender_id="u_1",
                content="市场活动页的视觉稿我已经补完了，等你这边确认。",
            ...
            Message(
                ...
                sent_at=now - timedelta(days=1, minutes=20),
            ),
        ]
    )
    db.commit()
    ...
    db.add_all(
        [
            ...
            Badge(
                ...
                description="月度全勤",
                icon="calendar",
                earned_at=now - timedelta(days=35),
            ),
        ]
    )
    db.commit()
    ...
    db.add_all(
        [
            ...
            Notification(
                ...
                user_id="u_me",
                title="工资条已生成",
                content="2026 年 02 月工资条已生成，可前往我的页面查看。",
                type="PROFILE",
                biz_id="s_2026_02",
                is_read=True,
            ),
